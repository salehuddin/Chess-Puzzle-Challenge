    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="space-y-16 lg:space-y-20">
            @foreach($steps as $i => $step)
                <div class="reveal bg-brand rounded-3xl p-8 lg:p-12 shadow-warm-lg ring-1 ring-neutral-900/5">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 {{ $i % 2 === 1 ? 'lg:[direction:rtl]' : '' }}">
                        ...
                        <div class="lg:sticky lg:top-24 lg:h-fit [direction:ltr]">
                            ...
                            @elseif($step['mockup'] === 'hall-of-fame')
                                {{-- Mockup: Hall of Fame sticker grid --}}
                            ...
                            @elseif($step['mockup'] === 'medal')
                                ...
                                <div class="absolute inset-0 bg-neutral-900 flex items-center justify-center p-6">
                                    <div class="text-center">
                                        <div class="w-32 h-32 mx-auto rounded-full bg-brand flex items-center justify-center text-6xl shadow-warm-lg ring-4 ring-white/10">
                                            🏅
                                        </div>
                    ...
                    {{-- Copy column --}}
                    <div class="reveal-right lg:pt-8 [direction:ltr]">
                        <div class="flex items-center gap-3 mb-5">
                            <div class="bg-neutral-900 text-brand rounded-full w-12 h-12 flex items-center justify-center text-sm font-black ring-4 ring-brand/30">
                                {{ $step['step'] }}
                            </div>
                            <span class="text-3xl">{{ $step['icon'] }}</span>
                        </div>
                        <h3 class="font-display text-3xl lg:text-4xl font-black text-neutral-900 mb-5 leading-tight">{{ $step['title'] }}</h3>
                        <p class="text-neutral-700 text-lg leading-relaxed mb-6">{{ $step['desc'] }}</p>

                        <ul class="space-y-2.5">
                            @foreach($step['features'] as $feature)
                                <li class="flex items-center gap-3 text-sm text-neutral-800">
                                    <span class="text-neutral-900 font-bold">{{ is_string($feature[0]) && mb_strlen($feature[0]) <= 3 ? $feature[0] : $feature[0] }}</span>
                                    <span>{{ $feature[1] }}</span>
                                </li>
                            @endforeach
                        </ul>
                        ...
                        @if($i === 3)
                            <a href="{{ url('/challenges') }}" class="inline-flex items-center gap-2 mt-8 btn btn-primary group">
                                Browse Challenges
