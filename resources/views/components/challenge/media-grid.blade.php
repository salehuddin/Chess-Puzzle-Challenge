...
@if($images !== [])
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 {{ $columns }}">
        @foreach($images as $i => $src)
            <figure class="group relative overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-warm">
                <img
                    src="{{ $src }}"
                    alt="{{ $alt }} #{{ $i + 1 }}"
                    loading="lazy"
                ...
                >
                <figcaption class="absolute inset-x-0 bottom-0 translate-y-full bg-gradient-to-t from-neutral-900/80 to-transparent p-4 text-xs font-semibold text-white transition duration-300 group-hover:translate-y-0">
                    {{ $alt }} #{{ $i + 1 }}
                </figcaption>
            </figure>
        @endforeach
