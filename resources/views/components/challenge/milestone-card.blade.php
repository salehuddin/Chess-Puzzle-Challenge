...
<div class="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-stone-200 bg-white p-6 shadow-warm transition hover:-translate-y-1 hover:shadow-warm-lg">
    <div class="flex items-start justify-between gap-3">
        <div>
            <p class="text-xs font-semibold uppercase tracking-[0.2em] text-amber-700">
                Milestone {{ (int) $sequence }}<span class="text-stone-400"> / {{ (int) $total }}</span>
            </p>
            <h3 class="mt-2 font-display text-xl font-black text-stone-900 line-clamp-2">
                {{ $title }}
            </h3>
        </div>
        <div class="flex flex-col items-end gap-1.5">
            <span class="grid h-12 w-12 place-items-center rounded-full bg-amber-50 font-display text-lg font-black text-amber-700 ring-1 ring-amber-200">
                {{ (int) $sequence }}
            </span>
            @if($isFirst)
    ...
    @if($themes !== [] || $rating !== null)
        <div class="mt-4 flex flex-wrap items-center gap-2">
            @if($rating !== null)
                <span class="badge badge-outline border-stone-300 text-xs font-semibold text-stone-600">
                    {{ (int) $rating }} ELO
                </span>
            @endif
            @foreach($themes as $theme)
                <span class="badge badge-ghost text-xs font-medium text-stone-600">
    ...
    <div class="mt-6 flex items-center gap-2 text-xs text-stone-500">
        <span class="inline-flex h-6 w-6 items-center justify-center rounded bg-base-200 text-base font-bold text-stone-600">♟</span>
