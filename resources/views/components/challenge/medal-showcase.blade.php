...
@if($medalArtworkUrl || $medalImages !== [])
    <div class="grid grid-cols-1 gap-10 lg:grid-cols-2 lg:items-start">
        <div class="lg:sticky lg:top-28">
            <div class="relative overflow-hidden rounded-3xl border border-amber-200 bg-gradient-to-br from-amber-50 via-white to-amber-100 p-8 shadow-warm-lg">
            ...
            <div class="mt-6 grid grid-cols-3 gap-3 text-center">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-widest text-stone-400">Finisher</p>
                    <p class="mt-1 font-display text-lg font-black text-stone-900">Medal</p>
                </div>
                <div>
                    <p class="text-xs font-semibold uppercase tracking-widest text-stone-400">Custom</p>
                    <p class="mt-1 font-display text-lg font-black text-stone-900">Design</p>
                </div>
                <div>
                    <p class="text-xs font-semibold uppercase tracking-widest text-stone-400">Free</p>
                    <p class="mt-1 font-display text-lg font-black text-stone-900">Shipping</p>
                </div>
            </div>
        </div>
        ...
        <div>
            @if($medalArtworkUrl)
                <figure class="overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-warm">
            ...
            @if($medalImages !== [])
                <div class="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-3">
                    @foreach($medalImages as $i => $src)
                        <figure class="group overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-warm" wire:key="medal-{{ md5($src) }}">
                            <img
                                src="{{ $src }}"
            ...
            <div class="mt-8 rounded-2xl border border-amber-200 bg-amber-50 p-5">
                <p class="text-sm font-semibold text-amber-900">Hand-crafted and shipped with care.</p>
